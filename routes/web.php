...
use App\Http\Controllers\AiCheckupController;
...
// AI CheckUp routes
Route::get('/ai-checkup', [AiCheckupController::class, 'index'])->name('ai.checkup');
Route::post('/ai-checkup/analyze', [AiCheckupController::class, 'analyze'])->name('ai.analyze');
...
// Chat API route
Route::post('/chat/message', [App\Http\Controllers\User\ChatController::class, 'handleMessage']);
...
Route::get('/ai-chatbot', function () {
    return view('user.ai_chatbot');
})->name('ai.chatbot');

// AI Chatbot API Endpoint
Route::post('/ask', function (Request $request) {
    // Kiểm tra xem yêu cầu có chứa tin nhắn không
    if (!$request->has('message')) {
        return response()->json(['response' => 'Vui lòng nhập câu hỏi.'], 400);
    }

    $message = $request->input('message');
    $useAiQuery = $request->input('use_ai_query', false) ? 'true' : 'false';

    // Chuẩn bị dữ liệu để gửi đến Flask server
    $data = [
        'message' => $message,
    ...
    // Gọi Flask API
    $flaskEndpoint = env('FLASK_API_URL', 'http://localhost:5000/ask');
    
    try {
        $client = new \GuzzleHttp\Client();
        $response = $client->post($flaskEndpoint, [
            'form_params' => $data,
            'timeout' => 30
        ]);
        ...
        return response()->json([
            'response' => 'Xin lỗi, không thể kết nối đến hệ thống AI. Vui lòng thử lại sau.',
            'error' => $e->getMessage()
        ], 500);
    }
})->name('ai.ask');
